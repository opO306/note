...
export function AttributionsScreen({ onBack }: AttributionsScreenProps) {
    return (
        <div className="w-full h-full bg-background text-foreground overflow-y-auto scrollbar-hide">
            {/* Header */}
            <header className="bg-card/95 backdrop-blur-xl border-b border-border sticky top-0 z-20 safe-top">
                <div className="px-4 py-3">
                    <div className="flex items-center space-x-3">
                        <Button
                            variant="outline"  // ✅ 박스형 버튼으로
                            size="icon"
                            onClick={onBack}
                            className="
                                touch-target
                        ...
                        <h1 className="flex items-center gap-2">
                            <ImageIcon className="w-5 h-5" style={ICON_STYLE} />
                            이미지·아이콘 출처
                        </h1>
                    </div>
                </div>
