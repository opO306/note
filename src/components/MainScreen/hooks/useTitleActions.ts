...
export function useTitleActions({
  ...
  const equipTitle = async (titleId: string) => {
    ...
    // 2) 로그인 유저 uid 가져오기
    const uid = auth.currentUser?.uid;
    if (!uid) return; // 로그인 안 되어 있으면 그냥 화면만 바꾸고 끝

    try {
      // 3) users/{uid} 문서의 currentTitle 필드 업데이트
      await updateDoc(doc(db, "users", uid), {
        currentTitle: titleId,
      });
    } catch (error) {
      console.error("[titles] currentTitle 저장 실패:", error);
      // 필요하면 여기서 토스트로 알려줘도 됨
    }
  };
  ...
  const unequipTitle = async () => {
    ...
    try {
      await updateDoc(doc(db, "users", uid), {
        currentTitle: "", // 또는 null
      });
    } catch (error) {
      console.error("[titles] currentTitle 해제 저장 실패:", error);
    }
  };
