...
export function MyPageScreen({
  ...
  // MyPageScreen 함수 안, useState 바로 아래쯤에 추가
  useEffect(() => {
  ...
  const DESCRIPTION_LIMIT = 80; // 권장 80~120 중 80자로 설정
  const [showSettings, setShowSettings] = useState(false);

  // 🔹 법적 문서 화면에서 돌아왔을 때 자동으로 설정 화면 열기
  useEffect(() => {
    if (autoOpenSettings) {
  ...
  const titles = [
    { id: "guide_sprout", name: "길잡이 꿈나무", requiredGuideCount: 1 },  // 🔼 0 → 1
    { id: "little_guide", name: "꼬마 길잡이", requiredGuideCount: 1 },
    { id: "ordinary_guide", name: "평범한 길잡이", requiredGuideCount: 5 },
    { id: "kind_guide", name: "친절한 길잡이", requiredGuideCount: 15 },
    { id: "famous_guide", name: "유명한 길잡이", requiredGuideCount: 30 },
    { id: "master_on_path", name: "길 위의 스승", requiredGuideCount: 50 },
    { id: "sherpa", name: "셰르파", requiredGuideCount: 100 },
    { id: "immortal_lantern", name: "불멸의 등불", requiredGuideCount: 200 },
  ];


  const getCurrentTitleName = () => {
    const titleData = titles.find(t => t.id === currentTitle);
    return titleData?.name || ""; // 기본 이름 없이 빈 문자열
  };

  const mockUserPosts = userPosts;
  const mockUserReplies = userReplies;
  const _RECENT_DAYS = 3;
  ...
  const recentUserPosts = userPosts
    ? [...userPosts].sort((a, b) => (b.lanterns || 0) - (a.lanterns || 0)).slice(0, 1)
    : [];

  const recentUserReplies = userReplies
    ? [...userReplies].sort((a, b) => (b.lanterns || 0) - (a.lanterns || 0)).slice(0, 1)
    : [];
  ...
  const handleTempDescriptionChange = useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      const isComposing = (e.nativeEvent as any)?.isComposing; // 한글 조합 중?
  ...
  const handleDescriptionOpenChange = useCallback((open: boolean) => {
    ...
    if (open) {
      setTempDescription(profileDescription); // 현재 값으로 채움
    }
  }, [profileDescription]);
  ...
  const handlePostsCardClick = useCallback(() => {
    if (onMyPostsCardClick) {
      // 부모(MainScreen)에 "내 글 전체 화면 열어줘" 요청
      onMyPostsCardClick();
    } else {
      // 부모에서 콜백 안 넘겨주면, 기존처럼 다이얼로그 사용
  ...
  const handleRepliesCardClick = useCallback(() => {
    if (onMyRepliesCardClick) {
      // 부모(MainScreen)에 "내 답글 전체 화면 열어줘" 요청
      onMyRepliesCardClick();
    } else {
      // 백업: 예전 방식 유지
  ...
  if (showSettings) {
    return (
      <SettingsScreen
        ...
        onLogout={onLogout}         // 🔹 계정 탈퇴 시 쓸 로그아웃 함수 전달
      />
    );
  }
      ...
      >
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                // 여기서는 화면 상단 핵심 프로필이라 eager 로딩 그대로 사용
                // loading="eager"
                // decoding 은 기본값 "async"
                />
              ...
              <div className="flex-1 min-h-20 flex flex-col">
                <div className="flex items-center justify-between">
                  ...
                  <Dialog open={showDescriptionDialog} onOpenChange={handleDescriptionOpenChange}>
                    ...
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>프로필 설명</DialogTitle>
                        <DialogDescription>
          ...
          <CardContent className="p-4 flex flex-col items-center text-center gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">
                신뢰도{" "}
                {/* ✅ 동적 색상만 사용하도록 text-amber-600 제거 */}
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-primary">
                        {topUserReply.postAuthor}님의 글에 답글
                      </span>
                      <span className="text-xs text-muted-foreground">
        ...
        <div className="space-y-2 mt-6">
          ...
          <Button
            ...
            onClick={handleLogoutClick}   // 🔹 여기만 변경
          >
