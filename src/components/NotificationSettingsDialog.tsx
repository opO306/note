...
export function NotificationSettingsDialog({
  ...
  const saveSettingsToLocal = useCallback(
    (enabled: Set<string>, allEnabled: boolean) => {
      ...
      safeLocalStorage.setItem("notificationSettings", JSON.stringify(arr));
      safeLocalStorage.setItem("allNotificationsEnabled", allEnabled.toString());
    },
    [],
  );

  // 🔹 Firestore + localStorage 동시 반영 헬퍼
  const persistSettings = useCallback(
    async (enabled: Set<string>, allEnabled: boolean) => {
      saveSettingsToLocal(enabled, allEnabled);
  ...
  useEffect(() => {
    ...
    const savedAllEnabled = safeLocalStorage.getItem("allNotificationsEnabled");
        ...
        if (snap.exists()) {
          ...
          if (ns && typeof ns === "object") {
            ...
            if (!cancelled) {
              ...
              saveSettingsToLocal(nextEnabled, serverAll);
            }
          } else {
            // 필드가 없는 경우 → 기본값으로 초기화
            if (!cancelled) {
