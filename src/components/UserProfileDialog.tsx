...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "./ui/tabs"; // ⬅️ 그대로 사용
...
import { FollowListScreen } from "./FollowListScreen";         // ★ 추가: 선원/승선한 배 전체 화면
import { MyContentListScreen } from "./MyContentListScreen";   // ★ 추가: 작성 글/답글 전체 화면
...
interface UserProfileScreenProps {
  ...
  userBio?: string;         // 🔹 프로필 한 줄 소개(마이페이지에서 쓰는 것 그대로)
  posts?: any[];
  ...
  isMyself?: boolean;
  isFollowing?: boolean;
  onToggleFollowUser?: (targetUserName: string) => void;

  // 🔹 팔로우 목록에서 유저를 눌렀을 때
  onFollowUserClick?: (nickname: string) => void;
  activeSection?: "profile" | "followers" | "following" | "posts" | "replies";
  onChangeSection?: (section: "profile" | "followers" | "following" | "posts" | "replies") => void;
}
...
export function UserProfileDialog({
  ...
  const setSection =
    onChangeSection ?? setInternalSection;
  ...
  const userData = useMemo(() => {
    // 1) 기본 통계 계산
    const totalPosts = posts.length; // 이 유저가 쓴 글 개수
    let totalLanterns = 0;          // 글에 받은 등불 합계
    let totalComments = 0;          // 글에 달린 답글 합계

    let bestPost: any = null;       // 가장 등불 많이 받은 글
  ...
  // 🔹 인기 글 카드 클릭 핸들러 (useCallback으로 메모이제이션)
  ...
  // 🔹 인기 답변 카드 클릭 핸들러 (useCallback으로 메모이제이션)
  ...
  const handleFollowButtonClick = useCallback(() => {
    if (!onToggleFollowUser || isMyself) return;
    onToggleFollowUser(displayName);
  }, [onToggleFollowUser, isMyself, displayName]);
  ...
  return (
    // 전체는 화면을 다 쓰되,
    // 안쪽 내용은 max-w-md 로 "모바일 폭"에 맞춰서 가운데 정렬
    <div className="w-full h-full bg-background text-foreground flex flex-col overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <div className="text-xs text-muted-foreground">달성한 업적</div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <div className="text-xs text-muted-foreground">보유 칭호</div>
              </CardContent>
            </Card>
          </div>
